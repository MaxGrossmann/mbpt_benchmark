...
def standardize_cell(structure):
    ...
    """
    # atomic positions have to be specified by fractional positions for spglib
    lattice = structure.lattice.matrix
    frac_positions = structure.frac_coords
    numbers = [i.specie.Z for i in structure.sites]
    cell = (lattice, frac_positions, numbers)
    lattice, frac_positions, numbers = spglib.standardize_cell(
        cell, to_primitive=True, symprec=1e-5, # same tolerance that 'blm' uses internally
    )
    s = Structure(lattice, numbers, frac_positions)
...
def plot_bs(ax, bs, lcs="k-", deco=True, cflag=[0, 0, 0], lw=1.0):
    """
    Useful function when plotting band structures.
    INPUT:
        ax:             Figure axis
        bs:             Band structure object (output of the 'parse_bs()' function)
        ...
        lw:             Line width when plotting without projections
    """
...
def create_pqmap(nnodes, ncores, fill=0.8, bse_flag=False, max_time=300.0, num_mpi=4):
    """
    Improve parallelization of the 'lmsig' and 'lbse' code using an automatically generated 'pqmap'/'pqmap-bse'.
    ...
    INPUT:
        ...
        bse_flag:       True = BSE 'pqmap'; False = self energy 'pqmap'
        max_time:       Maximum seconds the code can take to create a 'pqmap'
        num_mpi:        Preferred/maximum number of total MPI tasks
    OUTPUT:
        mpi_str:        Parallelization instructions for 'lmgw.sh'
    """
    # guard against uneven core split across nodes
    if ncores % nnodes != 0:
        sys.exit(f"'ncores' ({ncores:d}) must be divisible by 'nnodes' ({nnodes:d})!")
    # number of cores per node
    ncores_per_node = ncores // nnodes
    """
    Strategy for choosing an MPI+OpenMP layout:
    - Do not use more MPI tasks than the total number of cores available
    - The minimum number of OpenMP threads per MPI task is 1
    - On a single node, 4 MPI tasks with 1-16 threads each is usually fine
    - Multiple nodes: prefer fewer ranks per node to ease memory usage
      (e.g., 2 nodes -> 2 ranks/node; > 2 nodes -> 1 rank/node)
    - Cap by 'num_mpi', ensure 1 or more rank/node, and avoid oversubscription
    """
    if num_mpi < nnodes:
        print(
            f"    [warning] 'num_mpi={num_mpi:d}' < 'nnodes={nnodes}'. "
            f"At least one MPI task per node is required. Overriding the 'num_mpi={num_mpi:d}' cap and proceeding with {nnodes} total ranks.",
            flush=True,
        )
    tasks_per_node = max(1, min(num_mpi, ncores) // nnodes)
    threads_per_task = max(1, ncores_per_node // tasks_per_node)
    np_total = tasks_per_node * nnodes 
    def change_m_str(command_str):
        """
        Helper function to adjust command strings containing 
        'mpirun' for use on a SLURM-based cluster using 'srun'.
        """
        n = [int(i) for i in re.findall(r"\d+", command_str)]
        n = [n[0], n[2]]
        return f"srun --export=ALL,OMP_NUM_THREADS={n[0]:d},MKL_NUM_THREADS={n[0]:d} -c {n[0]:d} -n {n[1]:d}"
    # get the global execution mode
    execution_mode = get_execution_mode()
    # log message
    header = "pqmap-bse" if bse_flag else "pqmap"
    print(
        f"    Generating a '{header:s}' for {ncores_per_node:d} cores/node on {nnodes:d} node{'s' if nnodes != 1 else ''}\n"
        f"    (Total number of MPI tasks = {np_total:d} | MPI tasks per node = {tasks_per_node:d} | OpenMP threads per MPI task = {threads_per_task:d}).",
        flush=True,
    )
    # underutilization warning
    if np_total * threads_per_task < ncores:
        print(
            f"    [warning] Using only {np_total * threads_per_task:d} of {ncores:d} cores "
            f"({np_total:d} MPI x {threads_per_task:d} threads). Consider adjusting 'ncores', 'nnodes', or 'num_mpi'.",
            flush=True,
        )
    # clean up existing 'pqmap'/'pqmap-bse' files and a batch scripts
    if bse_flag:
        execute_command("rm -f pqmap-bse-[0-9]* batch-[0-9]*")
    else:
        execute_command("rm -f pqmap-[0-9]* batch-[0-9]*")
    # create the 'pqmap'/'pqmap-bse' and a batch scripts
    execute_command(f"lmfgwd ctrl.mat --job=-1 > {header:s}.log")
    while True:
        if fill < 0.5:
            ...
            if bse_flag:
                ...
                execute_command("rm -f pqmap-[0-9]* batch-[0-9]*")
            m1 = f"env OMP_NUM_THREADS={threads_per_task:d} MKL_NUM_THREADS={threads_per_task:d} mpirun -n 1"
            mn = f"env OMP_NUM_THREADS={threads_per_task:d} MKL_NUM_THREADS={threads_per_task:d} mpirun -n {np_total:d}"
            ml = mn
            md = mn
            mc = mn
        ...
        if bse_flag:
            pq_str = (
                ...
                f"--batch~bsw~np={np_total:d}~pqmap@fill={fill:.1f}~nodes={nnodes:d}~vanilla{ncores_per_node:d}"
            )
        else:
            pq_str = (
                f"lmfgwd ctrl.mat --job=0 --lmqp~rdgwin " +
                f"--batch~np={np_total:d}~pqmap@fill={fill:.1f}~nodes={nnodes:d}~vanilla{ncores_per_node:d}"
            )
        timeout_flag = execute_command_timeout(pq_str, f"{header:s}.log", max_time=max_time)
        with open(f"{header:s}.log", "r") as f:
    ...
    with open(f"batch-{np_total:d}", "r") as f:
        batch_str = f.read()
...
def get_bse_vb_manifold(bs, vbm_idx, lowest_energy=10, manifold_tolerance=0.5):
    ...
    (which is set to zero by definition, see 'parse_bs()'), the number of valence 
    bands whose maximum energy is within the specified range from the VBM.
    INPUT:
        bs:                     Band structure object (see 'parse_bs()' above)
