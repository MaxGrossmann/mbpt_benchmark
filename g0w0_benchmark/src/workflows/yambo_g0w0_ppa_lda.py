...
def yambo_g0w0_ppa_lda(cse, base_dir, ncores, calc_dir, db_entry_path):
    ...
    # workflow directory
    wf_dir = os.path.join(calc_dir, id, "yambo_g0w0_lda")
    ...
    bnd_step = cse.parameters["gw_conv_lda"]["gw_bnd_step"]
    cut_step = cse.parameters["gw_conv_lda"]["gw_cut_step"]
    kppa_step = 10
    bnd_final = int(cse.parameters["gw_conv_lda"]["w_conv_grid"][-1][0] + bnd_step)
    cut_final = int(cse.parameters["gw_conv_lda"]["w_conv_grid"][-1][1] + cut_step)
    kppa_scf = cse.parameters["qe_conv_lda"]["qe_conv_steps"][-1][1]
    kppa = cse.parameters["gw_conv_lda"]["conv_list"][-1][0]["kppa"]
    pw_cutoff = cse.parameters["gw_conv_lda"]["pwcutoff"]

    # increase the nscf/gw k-point density until the k-point grid changes once
    ...
    filename_scf = qe_runner.qe_pw_run(
        calc_data_scf, qe_write.write_pw, ncpu, kwargs={"input_dft": "lda"}
    )
    ...
    filename_nscf = qe_runner.qe_pw_run(
        ...
        qe_write.write_pw,
        ncpu,
        kwargs={"n_bands": bnd_final, "input_dft": "lda"},
    )
    ...
    cse.parameters.update({"g0w0_ppa_lda": gw_dict})
