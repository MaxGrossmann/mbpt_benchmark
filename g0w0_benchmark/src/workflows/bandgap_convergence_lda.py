...
given material using LDA pseudopotentials.
"""
...
def bandgap_convergence_lda(cse, base_dir, ncores, calc_dir, db_entry_path):
    ...
    # extract the convergence parameters from 'qe_convergence_lda'
    pw_cutoff = cse.parameters["qe_conv_lda"]["qe_conv_steps"][-1][0]
    kppa = cse.parameters["qe_conv_lda"]["qe_conv_steps"][-1][1]
    ...
    filename = qe_runner.qe_pw_run(
        ...
        qe_write.write_pw,
        ncores,
        kwargs={"input_dft": "lda", "disk_io": "high"},
    )
    fn.append(filename)
    ...
    cse.parameters.update(
        {"bg_conv_lda": param_dict, "indirect_gap_lda": bg_conv_steps[-1][2]}
    )
