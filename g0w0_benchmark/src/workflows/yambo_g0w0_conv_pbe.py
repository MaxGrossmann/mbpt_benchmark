...
starting from an PBE starting point. First, the convergence parameters for W are
...
def yambo_g0w0_conv_pbe(cse, base_dir, ncores, calc_dir, db_entry_path):
    ...
    # workflow directory
    wf_dir = os.path.join(os.getcwd(), "yambo_g0w0_conv_pbe")
    ...
    pw_cutoff_qe = cse.parameters["qe_conv_pbe"]["qe_conv_steps"][-1][0] # dummy value, get overwrittens in 'yambo_run_gw_conv_cs'
    kppa_qe = cse.parameters["qe_conv_pbe"]["qe_conv_steps"][-1][1] # used for initial scf in 'yambo_run_gw_conv_cs' 
    ...
    shutil.copytree("../pw_pbe/out", "out")
    ...
    cse.parameters.update({"gw_conv_pbe": gw_dict})
